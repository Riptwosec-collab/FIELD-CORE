...
public final class WifiScoutManager {
    ...
    public WifiScoutManager(Context c,EventSink sink){
        context=c.getApplicationContext();events=sink;wifi=(WifiManager)context.getSystemService(Context.WIFI_SERVICE);store=new WifiScoutStore(context);prefs=context.getSharedPreferences("fieldcore_wifi_scout_prefs",Context.MODE_PRIVATE);register();
    }
    ...
    @SuppressWarnings("deprecation") private void consumeResults(String source){
        ...
        List<Network> next=new ArrayList<>();Map<String,Integer> ssidCounts=new HashMap<>();
        ...
        next.sort((a,b)->Integer.compare(b.rssi,a.rssi));int newCount=0,open=0,hidden=0,secured=0,added=0,updated=0;
        for(Network n:next){seenBssid.add(n.bssid);Integer oldR=beforeRssi.get(n.bssid);if(oldR==null)added++;else if(Math.abs(oldR-n.rssi)>=5)updated++;n.duplicate=n.ssid.length()>0&&ssidCounts.getOrDefault(n.ssid,0)>1;JSONObject change=store.upsert(n.ssid,n.bssid,n.rssi,n.security,n.band,n.channel,n.hidden,now);n.isNew=change.optBoolean("new",false);n.securityChanged=change.optBoolean("securityChanged",false);n.trusted=change.optBoolean("trusted",false);n.risk=risk(n);if(n.isNew){newCount++;if(outdoorActive&&!sessionSeen.contains(n.bssid)){sessionNew++;emit("NEW_NETWORK",n.ssid,n.compact());}}if(n.securityChanged)emit("NETWORK_CHANGED",n.ssid+" SECURITY CHANGED",n.compact());if(n.trusted)emit("TRUSTED_NETWORK_FOUND",n.ssid,n.compact());if("OPEN".equals(n.security)){open++;if(n.isNew)emit("NEW_OPEN_WIFI",n.ssid,n.compact());}else secured++;if(n.hidden)hidden++;sessionSeen.add(n.bssid);}
    ...
    private int score(Network n){return scoreParts(n).optInt("total",0);}
