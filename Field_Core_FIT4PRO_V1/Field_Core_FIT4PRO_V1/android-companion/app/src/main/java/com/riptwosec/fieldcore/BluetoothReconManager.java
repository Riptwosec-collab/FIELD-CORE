...
public final class BluetoothReconManager {
    ...
    public void scan(Callback cb){
        ...
        current.clear();pending=cb;scanning=true;try{scanner.startScan(callback);}catch(SecurityException e){scanning=false;pending=null;cb.done(false,"BLUETOOTH PERMISSION REQUIRED",status());return;}catch(Exception e){scanning=false;pending=null;cb.done(false,"BLE SCAN ERROR",status());return;}main.postDelayed(()->stopScan("SCAN COMPLETE"),8000L);
    }
    ...
    static final class DeviceInfo{String address,name,type,proximity;int rssi;boolean trusted,isNew;long lastSeen;JSONObject compact(){JSONObject o=new JSONObject();try{o.put("n",name);o.put("a",address);o.put("r",rssi);o.put("type",type);o.put("prox",proximity);o.put("t",trusted);o.put("new",isNew);o.put("ts",lastSeen);}catch(Exception ignored){}return o;}}
}