...
/**
 * Keyless weather adapter backed by Open-Meteo. It never fabricates sensor or forecast data.
 * Network failures are returned as provider errors and the watch may continue using its cache.
 */
public final class WeatherProvider {
    ...
    public void execute(String action, Callback cb){
        location.lastKnown((ok,msg,loc)->{
            if(!ok||loc==null){cb.done(false,"WEATHER NEEDS PHONE LOCATION",null);return;}
            final double lat=loc.optDouble("latitude",Double.NaN);
            final double lon=loc.optDouble("longitude",Double.NaN);
    ...
    private void fetch(String action,double lat,double lon,JSONObject loc,Callback cb){
        ...
        try{
            ...
            String query=String.format(Locale.US,
                "?latitude=%.6f&longitude=%.6f&current=temperature_2m,apparent_temperature,relative_humidity_2m,pressure_msl,wind_speed_10m,wind_direction_10m,weather_code&daily=uv_index_max,sunrise,sunset,moonrise,moonset,moon_phase&forecast_days=1&timezone=auto",
                lat,lon);
            conn=(HttpURLConnection)new URL(base+query).openConnection();
            conn.setConnectTimeout(8000);conn.setReadTimeout(8000);conn.setRequestMethod("GET");conn.setRequestProperty("Accept","application/json");
            int code=conn.getResponseCode();
            if(code<200||code>=300){cb.done(false,"WEATHER HTTP "+code,null);return;}
            ...
            JSONObject root=new JSONObject(b.toString());JSONObject current=root.optJSONObject("current");JSONObject daily=root.optJSONObject("daily");
            JSONObject out=new JSONObject();out.put("source","OPEN_METEO");out.put("latitude",lat);out.put("longitude",lon);out.put("fetchedAt",System.currentTimeMillis());
            if(current!=null){
                copyNumber(current,out,"temperature_2m","temperatureC");copyNumber(current,out,"apparent_temperature","apparentC");
                copyNumber(current,out,"relative_humidity_2m","humidityPct");copyNumber(current,out,"pressure_msl","pressureMslHpa");
                copyNumber(current,out,"wind_speed_10m","windKmh");copyNumber(current,out,"wind_direction_10m","windDirectionDeg");
                copyNumber(current,out,"weather_code","weatherCode");
            }
            if(daily!=null){
                copyFirstNumber(daily,out,"uv_index_max","uvIndexMax");copyFirstString(daily,out,"sunrise","sunrise");copyFirstString(daily,out,"sunset","sunset");
                copyFirstString(daily,out,"moonrise","moonrise");copyFirstString(daily,out,"moonset","moonset");copyFirstNumber(daily,out,"moon_phase","moonPhase");
            }
            ...
            else if("THERMAL_REFRESH".equals(action)){
                double t=out.optDouble("temperatureC",Double.NaN),a=out.optDouble("apparentC",Double.NaN),uv=out.optDouble("uvIndexMax",0);
                out.put("heatLoad",heatLoad(t,a,uv));compact(out,new String[]{"source","temperatureC","apparentC","humidityPct","uvIndexMax","heatLoad","fetchedAt"});
            }
    ...
    private static String heatLoad(double t,double apparent,double uv){
        double v=Math.max(Double.isNaN(t)?0:t,Double.isNaN(apparent)?0:apparent);int score=0;
        if(v>=36)score+=3;else if(v>=32)score+=2;else if(v>=28)score+=1;if(uv>=8)score+=2;else if(uv>=6)score+=1;
