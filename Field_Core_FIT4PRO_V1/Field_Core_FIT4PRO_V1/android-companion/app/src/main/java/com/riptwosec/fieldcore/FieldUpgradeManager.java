...
public final class FieldUpgradeManager implements FieldEventBus.Listener {
    ...
    public void execute(String action,JSONObject payload,Callback cb){
        ...
        switch(action){
            ...
            case "OUTDOOR_SCAN_STOP": stopOutdoorService();JSONObject sum=wifi.stopOutdoor();try{String loc=prefs.getString("outdoor_session_location","");sum.put("locationAttached",loc!=null&&!loc.isEmpty());if(loc!=null&&!loc.isEmpty())sum.put("location",new JSONObject(loc));}catch(Exception ignored){}cb.done(true,"SCAN SUMMARY",sum);return;
            ...
            case "BT_RECON_HISTORY": cb.done(true,"DEVICE HISTORY",bluetooth.historyPage(p.optInt("limit",6)));return;
            ...
            case "ANCHOR_SUGGESTION": cb.done(true,"ANCHOR SUGGESTION",anchorSuggestion(p));return;
            ...
            case "EMERGENCY_ESCALATION_EVENT": cb.done(true,"EMERGENCY EVENT REVIEWED",reviewEmergencyEvent(p));return;
