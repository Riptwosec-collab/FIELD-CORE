...
export default {
  ...
  cancelEmergency(){if(this.emergencyTimer){clearInterval(this.emergencyTimer);this.emergencyTimer=null;}this.emergencyCountdown=0;this.impactPending=false;this.impactState={impactAt:0,stillSince:0,pending:false};this.view='detail';this.message='EMERGENCY CANCELED';this.logEvent('SOS','CANCELED');},
  triggerEmergency(reason){this.emergencyCountdown=0;this.impactPending=false;this.view='detail';this.openFeature('21');this.message='SOS LOCATION SENDING';this.logEvent('SOS',reason+' TRIGGER');this.sendCommand('EMERGENCY_SEND',{reason:reason,battery:this.watchBattery});this.haptic('long');},
